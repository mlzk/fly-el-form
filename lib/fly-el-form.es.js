...
const _sfc_main = defineComponent({
  ...
  setup(props, context) {
    ...
    const formKeyAndName = ref({});
    const FlyFormRef = ref(null);
    const needOverWriteForm = ref(null);
    ...
    const collectFormContent = (data = []) => {
      ...
      data.forEach((item) => {
        ...
        if (Object.prototype.hasOwnProperty.call(item, "rules") && item.rules.length > 0) {
        ...
        if (item.required) {
          const requiredType = Object.prototype.hasOwnProperty.call(
            item,
            "requiredType"
          ) ? item.requiredType : requireTypes[item.type] || "string";
          const tips = {
            required: true,
          ...
          if (Object.prototype.hasOwnProperty.call(res.rules, item.key) && res.rules[item.key].length > 0) {
        ...
        } else if (Object.prototype.hasOwnProperty.call(item, "source") && Object.prototype.hasOwnProperty.call(item.source, "data") && item.source) {
          sourceData.value[item.key] = item.source.data;
        } else {
          !Object.prototype.hasOwnProperty.call(sourceData.value, item.key) && (sourceData.value[item.key] = []);
        }
        if (!item.placeholder) {
        ...
        if (item.type === "el-row" && Object.prototype.hasOwnProperty.call(item, "children") && item.children.length > 0) {
      ...
      return async function() {
        try {
          ...
          if (effectKeys && effectKeys.length > 0) {
            ...
            requestParams = requestParams ? { requestParams, ...effectKeysFormValues } : effectKeysFormValues;
          ...
          const res = requestParams ? await requestFunction(requestParams) : await requestFunction();
  ...
  render(props) {
    ...
    const generatorTitle = (item) => {
      const props2 = item.props ? item.props : {};
      return h(
        resolveComponent("el-row"),
        {
          ...props2
    ...
    const generatorForm = (data) => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (["el-row", "Title"].includes(item.type)) {
          ...
          if (hasOwnPropertySafely(item, "visitable") && item.visitable(status, this.formValues) || !hasOwnPropertySafely(item, "visitable")) {
    ...
    const generatorSelect = (item) => {
      var _a2;
      const formRef = this;
      const selectedLabel = this.formValues[item.key] ? (_a2 = this.sourceData[item.key].find(
        (option) => option[item.showValue || "value"] === this.formValues[item.key]
      )) == null ? void 0 : _a2[item.showName || "label"] : "";
      let defaultEvent = {
        "onUpdate:modelValue": (val) => {
          this.formValues[item.key] = val;
        }
      ...
      return h(
        ...
        {
          default: () => item.custom && item.custom.group ? generatorOptionsGroup(item, this.sourceData[item.key]) : generatorOptions(item, this.sourceData[item.key]),
          // 使用函数式插槽来渲染 label
