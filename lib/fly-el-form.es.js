  ...
  render(props) {
    ...
    const generatorSelect = (item) => {
      ...
      const getSelectedLabel = () => {
        ...
        if (item.custom && item.custom.group) {
          ...
          return ((_a2 = sourceDataArray.find(
            (option) => option[item.showValue || "value"] === this.formValues[item.key]
      ...
      return h(
        ...
        {
          default: () => item.custom && item.custom.group ? generatorOptionsGroup(item, sourceData) : generatorOptions(item, Array.isArray(sourceData) ? sourceData : []),
    ...
    const generatorOptions = (propItem, data = []) => {
      ...
      return data.map((item) => {
        ...
        const slots = propItem.optionSlot && typeof propItem.optionSlot === "function" ? { default: () => propItem.optionSlot(item, h) } : { default: () => label };
        return h(
