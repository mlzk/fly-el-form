...
const FlyElForm = defineComponent({
	...
	setup(props, context) {
		...
		const formKeyAndName: Record<string, any> = reactive({})
		...
		// 获取组件引用的方法
		const getComponentRefByKey = (key: string) => {
		...
		// 辅助函数：简单的深比较，生产环境可以考虑使用 lodash.isEqual
			...
			data.forEach((item: FlyFormTypes.FormItem) => {
				...
				if (hasOwnPropertySafely(item, 'key')) {
					if (hasOwnPropertySafely(res.formKeyAndName, item.key)) {
						// 检查本次收集的 formKeyAndName 是否有重复 key
						throw new Error(
							`form item key "${item.key}" is duplicated in the current form definition. Please use another key.`
						)
					} else {
						res.formKeyAndName[item.key] = item.name // 收集到临时的 res.formKeyAndName 中
					}
		...
		// 新增：提供表单值上下文
		provide(FormValuesSymbol, formValues)
		return {
			isFirstInit,
			formContent,
			requests,
	...
	render(props: any) {
			...
			if (
				...
				if (item.slot) {
					return h(
						...
						{
							ref: this.setComponentRef(item.key), // 使用 ref 绑定
		...
		const generatorSelect = (item: FlyFormTypes.FormItem) => {
			...
			if (item.type === 'el-select-v2') {
				defaultProps.default = (optionItem: any) =>
					item.optionSlot ? item.optionSlot!(optionItem, h) : null
		...
		const generatorOptions = (
			...
			return data.map((item) => {
				const value = propItem.custom?.returnObject
					? item
					: item[propItem.showValue || 'value']
				...
				const slots =
					propItem.optionSlot && typeof propItem.optionSlot === 'function'
			...
			if (['el-date-picker'].includes(item.type)) {
				return {
					'on-change': (val: any) => {
						// 先更新值
						this.formValues[item.key] = val
						// 如果有 effectKeys，更新相关数据源
						if (item.effectKeys && item.effectKeys.length > 0) {
							this.updateRequestSource(item.effectKeys)
						}
						// 执行用户自定义的 change 事件
						item.componentEvents?.['on-change']?.(val)
					},
				}
			} else {
				return {
					'onUpdate:modelValue': (val: any) => {
						...
						// 执行用户自定义的 update 事件
						item.componentEvents?.['onUpdate:modelValue']?.(val)
					},
				}
			}
		}
